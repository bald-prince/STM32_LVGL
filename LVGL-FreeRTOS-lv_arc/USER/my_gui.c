...
lv_obj_t *label_left = NULL;            /* 左侧百分比标签 */
lv_obj_t *label_right= NULL;           /* 右侧百分比标签 */
lv_obj_t *arc_left = NULL;              /* 左侧圆弧 */
lv_obj_t *arc_right = NULL;             /* 右侧圆弧 */
...
static void timer_cb(lv_timer_t *timer)
{
	if(val < 100)
	{
		...
		lv_label_set_text_fmt(label_left, "%d%%", lv_arc_get_value(arc_left));    /* 获取当前值，更新左侧百分比标签 */
    lv_label_set_text_fmt(label_right, "%d%%", lv_arc_get_value(arc_left));   /* 获取当前值，更新右侧百分比标签 */
	}
	else{
		val = 0;
	}
